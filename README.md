...
**Icons**: the manifest declares `/public/logo.svg` for all sizes. Android Chrome and modern desktop browsers install fine with this. **For native-quality iOS PWA install**, drop a 1024×1024 `apple-touch-icon.png` at `/public/apple-touch-icon.png` (any SVG→PNG tool — Figma export, Squoosh, `sharp`, etc.) and the `icons` field in `layout.tsx` will pick it up automatically.
